...
- **Language** — Speak in `the workspace setting `communication_language``, tailored to `the workspace setting `user_skill_level``. Write files in `the workspace setting `document_output_language``.
...
1. Load context.
   ...
   - **Determine context strategy.** Using the intent and the artifact listing, infer whether the current work is a story from an epic. Do not rely on filename patterns or regex — reason about the intent, the listing, and any epics file content together.
     ...
     **A) Epic story path** — if the intent is clearly an epic story:
     ...
     3. **Compile epic context if needed.** If no valid cached epic context was loaded, produce `the workspace setting `implementation_artifacts`/epic-<N>-context.md` by spawning a subagent synchronously with `` as its prompt. Pass it the epic number, epics file path, `the workspace setting `planning_artifacts``, and output path `the workspace setting `implementation_artifacts`/epic-<N>-context.md`.
