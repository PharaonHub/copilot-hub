...
1. **Folder+id dispatch** (`{spec_folder}` and `{story_id}` are set): the write-back always lands at the id-keyed story spec. The `the workspace setting `implementation_artifacts`` fallback in step 2 below is never used in this mode, even for halts before planning starts.
...
2. **Otherwise:**
   ...
   - If `{spec_file}` is unknown or missing, create `the workspace setting `implementation_artifacts`/bmad-build-auto-result-<slug-or-timestamp>.md` with:
     ```markdown
     ---
     status: <final status>
     ---
...
the workflow setting `on_complete`
...
- Speak in `the workspace setting `communication_language``, tailor communication to `the workspace setting `user_skill_level``, and write documents in `the workspace setting `document_output_language``.
...
the workflow setting `activation_steps_prepend`

### Step 2: Load Persistent Facts

Treat every entry below as foundational context you carry for the rest of the workflow run. Entries prefixed `file:` are paths or globs under `the workspace root` -- load the referenced contents as facts. All other entries are facts verbatim (`_None._` means none):

the workflow setting `persistent_facts`

### Step 3: Execute Append Steps

Execute each of these steps in order (`_None._` means skip):

the workflow setting `activation_steps_append`
