...
- Bare paths (e.g. `references/best-practices.md`) resolve from this skill's bundled directory.
- `the workspace root` → the project working directory.
- **Target** → the repository being described, defaulting to `the workspace root`. If it resolves to more than one working tree, or to one the user cannot commit in, ask before writing.

## On Activation

1. Read optional behavior from explicit caller inputs and use bundled assets as defaults. Execute any supplied activation steps; treat supplied persistent facts as standing context (`file:` = paths/globs to load, others verbatim).
2. Read user-provided name, language (use it every turn), and output folder from the conversation or workspace. Standalone: skip.
...
4. Detect intent and greet `the user-provided name`: **setup** (no instruction file in the target carries meaningful content — scaffolding alone, empty headings, a comment, a lone import line, is not meaningful; when unsure, adopt, since adopting a near-empty file costs one small ledger while setting up a meaningful one loses instructions), **adopt** (an instruction file has content but no managed block, whatever its state and whoever wrote it — the migration form of refresh; that file is the baseline and every instruction in it enters the ledger of step 1), **refresh** (a managed block exists), **record** (the user reports a mistake agents made), **audit** (re-verify and prune). A supplied intent that contradicts what detection finds — e.g. `setup` against a file with content — is surfaced and confirmed, never silently obeyed. Fold `the explicit external_sources input` into the source list. Execute `the explicit activation_steps_append input`.
