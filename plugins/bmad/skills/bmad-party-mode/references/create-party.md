...
A guided authoring flow that turns an idea — a themed cast, a one-off persona, or a pile of raw profile data — into custom party members and groups, written to the user's documented defaults override. The output is configuration; `bmad-customize` does the actual write.
