...
The log is append-only and grows every session, so don't pull the raw file into the party. Hand a reader subagent the memlog path (`the user's explicit memory_dir input or the documented default/{active}/.memlog.md`) and have it return a compact brief — a few hundred tokens of *where things stand now*, ready to play in character.
...
```
uv run "<installed bmad-deep-recon skill directory>/scripts/recon_kit.py" memlog append \
  --workspace the user's explicit memory_dir input or the documented default/{active} \
  --type <dynamic|moment|callback|outcome> \
  --text "<one succinct line, in the room's own read of it>"
```

Add `--by <persona-code>` when a memory belongs to one character. Choose `init` vs `append` from the existence fact you already hold: the entry-read (and, on a mid-session room switch, that room's read) told you whether the memlog exists — `init --workspace the user's explicit memory_dir input or the documented default/{active}` once before the first append when it doesn't, plain `append` when it does. (`init` errors if the file already exists, so don't call it blind.)

If the shared memlog helper is unavailable or a write errors, skip it silently and never stall the party on a failed write.

## Forget

The memlog is append-only by design — no surgical delete. To wipe a party's memory, delete its folder (`the user's explicit memory_dir input or the documented default/{active}/`). To correct a wrong memory, append a new entry that supersedes it; the room reads the latest state.
