...
- **Paths:** bare paths (e.g. `references/create-party.md`) resolve from `this skill directory` (where `documented defaults` lives); `the current VS Code workspace root`-prefixed paths from the project working dir. `the user's explicit <name> input or the documented default` resolves to `documented defaults`'s `[workflow]` table (overrides win).
Use explicit values from the request or neutral defaults; do not load generated configuration.
- **File roles:** a party's memory is the per-party memlog at `the user's explicit memory_dir input or the documented default/<party>/.memlog.md`; custom members and groups live in the user's `documented defaults` overrides. Mechanics in `references/party-memory.md` (memory) and `references/create-party.md` (authoring).
...
Use the user-supplied roster or the documented default party; keep it session-local.
Use the user-supplied roster or the documented default party; keep it session-local.
...
7. Run each `the user's explicit activation_steps_append input or the documented default` entry; if either hook list was non-empty, confirm every entry ran before continuing.
...
Use `the user's explicit party_mode input or the documented default` for the session unless the user passed `--mode <session|auto|subagent|agent-team>` (the older `--subagents` means `subagent`) — runtime intent always wins. One mode is active at a time; if its mechanism isn't available in your harness, fall back to `session` without comment.
...
- Run `the user's explicit on_complete input or the documented default` if non-empty, then drop back to normal mode.
