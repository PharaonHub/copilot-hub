...
Subagent prompt. Produce one `.excalidraw` file at `.working/ia-the current date.excalidraw` (IA diagram) or `.working/flow-{name}-the current date.excalidraw` (flow wireframe).
