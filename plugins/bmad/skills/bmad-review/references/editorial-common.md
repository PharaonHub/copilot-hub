...
The baseline style guide is `the user's explicit style_guide input or the documented default`; a style guide stated in the request wins over the configured one for that run. Where the style guide in effect conflicts with a generic principle here — including the reader calibration — the style guide wins. Nothing overrides content being sacrosanct.

## Setup

1. Gather inputs: the content (required — a path or pasted text), plus whatever the request states: purpose, target audience, length target, reader type, style guide. If no reviewable content was provided, say so and stop. Request-level values win; `the user's explicit reader_type input or the documented default` and `the user's explicit style_guide input or the documented default` fill what the request leaves unstated. Treat `the user's explicit review_guidance input or the documented default` entries as standing review directives.
2. When the content is a file, get exact word counts — document total and per heading section — via `uv run this skill directory/scripts/word_metrics.py <path>` (`--help` documents the output), and ground every word-impact estimate and the reduction summary in those numbers. If the content was pasted or the script cannot run, estimate and mark the numbers as estimates.
...
Calibrate every finding to the reader type — stated in the request, else `the user's explicit reader_type input or the documented default`.
...
Shape the table per `the user's explicit output_preferences input or the documented default`.
