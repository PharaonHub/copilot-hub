...
```
uv run --no-cache "<resolved skill directory>/scripts/sprint_status.py" update \
...
Report where the document was saved, the verdict, and the action-item count. Then, if `the workflow setting `on_complete`` is non-empty, follow it as the final terminal instruction before exiting.
