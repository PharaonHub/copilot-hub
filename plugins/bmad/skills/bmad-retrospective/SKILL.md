...
- Bare paths and `this skill folder` (e.g. `references/aggregate-views.md`, `scripts/sprint_status.py`) resolve from this skill's installed directory.
- `the workspace root` → the project working directory.
- `the skill name` → the skill directory's basename.
...
1. **Load workflow inputs.** Use workflow settings supplied explicitly by the caller. This plugin does not bundle a workflow resolver or generated defaults; when a setting is absent, use the behavior described in this skill.
2. **Run prepend steps** — execute each entry in `the workflow setting `activation_steps_prepend`` in order.
3. **Load persistent facts** — treat every `the workflow setting `persistent_facts`` entry as standing context. `file:` entries are paths/globs under `the workspace root` whose contents load as facts; all others are literal facts.
4. **Load config** from `the workspace root/the workspace configuration`: `project_name`, `user_name`, `communication_language`, `document_output_language`, `user_skill_level`, `planning_artifacts`, `implementation_artifacts`, and `date` (system datetime), plus `output_folder` from `the workspace root/the workspace configuration`. Speak all output in `{communication_language}`; write all documents in `{document_output_language}`. Never state time estimates — AI has changed development speed, so hour/day/week predictions are noise.
...
6. **Run append steps** — execute each entry in `the workflow setting `activation_steps_append`` in order.
