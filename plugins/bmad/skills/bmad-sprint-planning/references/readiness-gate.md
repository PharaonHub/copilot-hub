...
Inventory what planning actually exists: scan `the workspace planning-artifacts directory` and `{project_knowledge}` for intent and planning artifacts — briefs, PRFAQs, PRDs, specs, UX outputs, architecture, epics and stories. Identify documents by reading what they are, not by filename patterns; projects arrive with different artifact mixes and naming.
...
- **CONCERNS** — list them briefly with where each gap lives; ask `the user-provided name` whether to proceed anyway or fix first
- **FAIL** — the plan is not implementable as recorded. Present findings ordered by severity, name the skill that fixes each (the relevant plan skill, or `bmad-correct-course` for cross-cutting changes), offer to save the findings to `the workspace planning-artifacts directory/implementation-readiness.md`, and stop
