...
Subcommands:
  ...
  memlog init --workspace WORKSPACE [--field KEY=VALUE ...]
      Create a workspace .memlog.md without overwriting an existing log.
  memlog append --workspace WORKSPACE --type TYPE --text TEXT [--date DATE]
      Append one normalized, quoted entry to a workspace .memlog.md.
  memlog set --workspace WORKSPACE --key KEY --value VALUE
      Set or replace a metadata comment in a workspace .memlog.md.
"""
...
def cmd_slug(args) -> int:
    ...
    folder = (args.pattern
              ...
              .replace("the current date", args.date or date.today().isoformat()))
...
MEMLOG_TYPE_RE = re.compile(r"^[a-z][a-z0-9-]*$")


def memlog_path(workspace: str) -> Path:
    return Path(workspace) / ".memlog.md"


def memlog_split(text: str) -> tuple[dict[str, str], str]:
...
def memlog_render(metadata: dict[str, str], body: str) -> str:
    frontmatter = "\n".join(
...
def memlog_touch(metadata: dict[str, str]) -> None:
    metadata.pop("updated", None)
    metadata["updated"] = datetime.now().strftime("%Y-%m-%dT%H:%M")


def memlog_write_atomic(path: Path, content: str) -> None:
...
def cmd_memlog_init(args) -> int:
    workspace = Path(args.workspace)
    workspace.mkdir(parents=True, exist_ok=True)
    path = memlog_path(args.workspace)
    if path.exists():
        return out({"workspace": str(workspace), "memlog": str(path), "created": False}, 0)
    metadata: dict[str, str] = {}
    for raw in args.field:
        ...
        if not re.fullmatch(r"[a-z][a-z0-9_-]*", key):
            print(f"error: invalid memlog metadata key: {key!r}", file=sys.stderr)
            return 2
        metadata[key] = value.strip()
    memlog_touch(metadata)
    memlog_write_atomic(path, memlog_render(metadata, ""))
    return out({"workspace": str(workspace), "memlog": str(path), "created": True}, 0)


def cmd_memlog_append(args) -> int:
    path = memlog_path(args.workspace)
    if not path.exists():
        print(f"error: memlog does not exist; run memlog init first: {path}", file=sys.stderr)
        return 2
    entry_date = args.date or date.today().isoformat()
    parse_date(entry_date)
    if not MEMLOG_TYPE_RE.fullmatch(args.type):
        print(f"error: invalid memlog type: {args.type!r}", file=sys.stderr)
        return 2
    try:
        metadata, body = memlog_split(path.read_text(encoding="utf-8"))
    except ValueError as error:
        print(f"error: {error}", file=sys.stderr)
        return 2
    line = f"- ({args.type}) date={entry_date} text={json.dumps(args.text, ensure_ascii=False)}"
    body = body.rstrip("\n")
    body = f"{body}\n{line}" if body else line
    memlog_touch(metadata)
    memlog_write_atomic(path, memlog_render(metadata, body))
    return out({"memlog": str(path), "type": args.type, "date": entry_date}, 0)


def cmd_memlog_set(args) -> int:
    path = memlog_path(args.workspace)
    if not path.exists():
        print(f"error: memlog does not exist; run memlog init first: {path}", file=sys.stderr)
        return 2
    key = args.key.strip()
    if not re.fullmatch(r"[a-z][a-z0-9_-]*", key):
        print(f"error: invalid memlog metadata key: {args.key!r}", file=sys.stderr)
        return 2
    try:
        metadata, body = memlog_split(path.read_text(encoding="utf-8"))
    except ValueError as error:
        print(f"error: {error}", file=sys.stderr)
        return 2
    metadata[key] = args.value
    memlog_touch(metadata)
    memlog_write_atomic(path, memlog_render(metadata, body))
    return out({"memlog": str(path), "key": key, "value": args.value}, 0)
...
def main(argv: list[str] | None = None) -> int:
    ...
    pg.add_argument("--pattern", default="{research_type}-{topic_slug}-the current date",
                    help="folder pattern (default: {research_type}-{topic_slug}-the current date)")
    ...
    pm = sub.add_parser("memlog", help="initialize or append a normalized session memory log")
    pm_sub = pm.add_subparsers(dest="memlog_cmd", required=True)

    pmi = pm_sub.add_parser("init", help="create .memlog.md without overwriting it")
    pmi.add_argument("--workspace", required=True, help="workspace folder for .memlog.md")
    pmi.add_argument("--field", action="append", default=[], metavar="KEY=VALUE",
                     help="optional metadata comment; may be repeated")
    pmi.set_defaults(func=cmd_memlog_init)

    pma = pm_sub.add_parser("append", help="append one typed entry")
    pma.add_argument("--workspace", required=True, help="workspace folder containing .memlog.md")
    pma.add_argument("--type", required=True,
                     help="lowercase event type, e.g. decision or assumption")
    pma.add_argument("--text", required=True)
    pma.add_argument("--date", help="entry date (YYYY-MM-DD; default today)")
    pma.set_defaults(func=cmd_memlog_append)

    pms = pm_sub.add_parser("set", help="set or replace metadata")
    pms.add_argument("--workspace", required=True, help="workspace folder containing .memlog.md")
    pms.add_argument("--key", required=True)
    pms.add_argument("--value", required=True)
    pms.set_defaults(func=cmd_memlog_set)
