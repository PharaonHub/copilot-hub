...
- **The memlog is the process memory.** Every decision, source batch, load-bearing claim, plan change, and assumption is one append-only line. Initialize and append it with the bundled helper: `uv run <skill-dir>/scripts/recon_kit.py memlog init --workspace <run-folder> --field topic="<product>"`, then `uv run <skill-dir>/scripts/recon_kit.py memlog append --workspace <run-folder> --type <decision|source|claim|assumption|question|event> --text "<one-line gist>"`. Do not hand-count claims; use the `tally` subcommand.
- Web access is required for Run. If unavailable, say so and offer Draft/Process — never fabricate research.

## Resolution rules

- Bare paths and `this skill directory` (e.g. `references/run.md`) resolve from this skill's installed directory.
- `the current VS Code workspace root` → the project working directory; `{skill-name}` → the skill directory's basename.
- `the user's explicit <name> input or the documented default` → a merged `documented defaults` field; `the explicit document workspace` → the bound run folder.
- Forward slashes only. Config variables already contain `the current VS Code workspace root` in their resolved values — never double-prefix.
...
3. Headless (no interactive user) → see `## Headless Mode`. Otherwise greet `the user-provided name` in `the user-provided language` — and stay in it every turn.
...
5. If a run folder for this topic already exists under `the user's explicit research_output_path input or the documented default`, offer to resume or extend it (a drafted brief awaiting its report, a report awaiting refresh) rather than start a duplicate.

## Research types and decision shapes

The type set is whatever `the user's explicit research_types input or the documented default` resolves to — shipped: `market`, `domain`, `technical`, `competitive`, `user-voice`, `academic-lit` — each pointing at `types/<research-type>.md` under this skill directory. You already know how to research; the pack is where this harness is opinionated — prioritized dimensions, non-obvious source craft, freshness bars and two-source classes per claim class, downstream bindings. Apply it in every mode; don't re-derive it. Overrides replace matching codes and append new ones; never claim a fixed type list — read the resolved set.
...
```json
{
  ...
  "intent": "run",
  "type": "market",
  "report": "the explicit document workspace/research.md",
