...
Assemble `research.md` in this order, shaped by `the user's explicit audience input or the documented default` and written in `the requested output language`:
...
Update the frontmatter (`status: complete`, `updated`, and the verified/unverified counts from `uv run scripts/recon_kit.py tally the explicit document workspace/.memlog.md` — never hand-counted), log a final `event` in the memlog, and proceed to `references/finalize.md`.
