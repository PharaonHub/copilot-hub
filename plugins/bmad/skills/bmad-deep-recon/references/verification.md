...
- **high** — cross-check every claim in the pack's *two-source classes*, and run the red-team pass on major conclusions regardless of `the user's explicit red_team input or the documented default`.
...
Verifier assistants run behind the research firewall on `the user's explicit subagent_models input or the documented default` when set; judgment work never drops to the smallest tier.
