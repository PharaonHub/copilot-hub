...
| Preset (`the user's explicit preset input or the documented default`) | subagents | sources/round | depth |
|---|---|---|---|
| `quick` | low (2) | 5 | 1 |
...
`the user's explicit subagent_models input or the documented default` is an ordered model preference for assistants — first available wins; empty means harness default. Keep the lead on the strongest model; researchers at most one tier down; judgment work never on the smallest tier.

## The plan gate

The one hard stop, kept light: decision, type and pack-derived dimensions pruned to it, shape, the **decomposition topology** — *breadth-first* (independent sub-questions: assistants split the dimensions), *depth-first* (one question that needs several perspectives: assistants split by angle or methodology, not by dimension), or *straightforward* (a focused ask: one assistant, a handful of calls, no fan-out — never overinvest in a simple query) — knobs in force and where each came from, which search surfaces exist (harness web search; installed search-shaped MCP tools; `the user's explicit external_sources input or the documented default` — check, don't assume), whether to run the fan-out as a workflow when the harness offers orchestration and `the user's explicit use_workflows input or the documented default` allows, and an honest time estimate (a standard run is minutes; deep runs are tens of minutes and many times the tokens).

Present as a compact checklist, get approval, then: bind `the explicit document workspace` under `the user's explicit research_output_path input or the documented default` — expand the folder name with `uv run scripts/recon_kit.py slug "<topic>" --type <type> --pattern "the user's explicit run_folder_pattern input or the documented default"` so the same topic always resolves to the same folder — seed `research.md` from `the user's explicit research_template input or the documented default`, init the memlog (`uv run "<installed bmad-deep-recon skill directory>/scripts/recon_kit.py" memlog init --workspace <the explicit document workspace> --field topic="<topic>" --field type="<type>" --field decision="<decision>" --field preset="<preset>"`), log the approved plan as a `decision`, and tell the user the path.

Each dimension then runs in **rounds** — up to the resolved `max_depth` — and the report grows as material lands: the user watches the document build, not a spinner. Every digest is written to `the explicit document workspace/digests/` the moment it exists — one file per assistant per round (`<dimension>-r<round>-<n>.md`), the digest shape below, raw enough to re-derive from.
...
- its search surfaces (specialized tools first — installed search-shaped MCP tools, `the user's explicit external_sources input or the documented default` entries whose directive matches — then generic search), plus `the user's explicit preferred_sources input or the documented default` first / `the user's explicit banned_sources input or the documented default` never
...
**On each return, write the digest to `the explicit document workspace/digests/` before doing anything else with it.**

Spawn assistants on `the user's explicit subagent_models input or the documented default` when set (first available wins); otherwise the harness default — judgment work never drops to the smallest tier. When subagents are unavailable (or `subagents` is `none`), run the same rounds yourself, sequentially, under the same budgets and the same files-first discipline.
