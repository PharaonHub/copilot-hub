...
> You are validating a PRD against the quality rubric at `the explicit validation_checklist_template input`. Read the full rubric first, then read `prd.md` (and `addendum.md` if present). Form a judgment per dimension — *strong / adequate / thin / broken* — and write findings only where they add information. Cite specific PRD locations and quote phrases. Severity ranks impact on the PRD's usefulness, not how easy the fix is. Write your review to `the explicit document workspace/review-rubric.md` in the format the rubric specifies. Return ONLY a compact summary (overall verdict, dimension verdicts, finding counts by severity, file path).

The Reviewer Gate may also dispatch additional reviewers from `the explicit finalize_reviewers input` (adversarial-general by default) and any ad-hoc reviewers the parent judges warranted. Each writes its review to `the explicit document workspace/review-{slug}.md` and returns a compact summary. Run in parallel.
...
- `the explicit document workspace/review-rubric.md` — primary, structured by the seven dimensions
- Zero or more `the explicit document workspace/review-{slug}.md` files — extra reviewers (adversarial, etc.)
- `the explicit validation_report_template input` — the HTML skeleton

### What the synthesis pass does

1. Read every reviewer file in `the explicit document workspace/review-*.md`.
...
3. Write the filled HTML to `the explicit document workspace/validation-report.html`.
4. Write the markdown twin to `the explicit document workspace/validation-report.md` (same content, grouped by severity rather than by dimension — see format below; this is the canonical form for downstream re-reading).
5. Open the HTML in the default browser with the platform opener — `open` on macOS, `xdg-open` on Linux, `start ""` on Windows — double-quoting the path:
   ```bash
   open "the explicit document workspace/validation-report.html"
   ```
