...
Follow `the referenced bundled workflow file` with `target_status` = `in-progress`.
...
the explicit workflow option `oneshot_review_layers`

If a layer's instruction requires subagents and none are available, for each such layer write under `the workspace setting `implementation_artifacts`` the exact child prompt from that layer's instruction after placeholder substitution (not a path-only pointer), then HALT. Ask the human to run each in a separate session and paste back the findings.
...
- **defer** — Defer every other entry, including pre-existing issues and improvement ideas. Append one new entry to `the workspace setting `implementation_artifacts`/deferred-work.md` using this format. Do not modify existing entries or look for duplicates.
  ```markdown
...
Write `{spec_file}` using `the referenced bundled workflow file`. Fill only these sections — delete all others:

1. **Frontmatter** — set `title: '{title}'`, `type`, `created`, `status: 'done'`. Add `route: 'one-shot'`.
2. **Title and Intent** — `# {title}` heading and `## Intent` with **Problem** and **Approach** lines. Reuse the summary you already generated for the terminal.
3. **Suggested Review Order** — append after Intent. Build using the same convention as `the referenced bundled workflow file` § "Generate Suggested Review Order" (spec-file-relative links, concern-based ordering, ultra-concise framing).
...
Follow `the referenced bundled workflow file` with `target_status` = `review`.
...
the explicit workflow option `open_spec`
...
the explicit workflow option `on_complete`
