...
- **Language** — Speak in `the workspace setting `communication_language``. Write any file output in `the workspace setting `document_output_language``.
...
1. Explicit argument
   ...
   - If it points to a file that matches the spec template (has `status` frontmatter with a recognized value: draft, ready-for-dev, in-progress, in-review, or done) → set `spec_file`. Before exiting, run **Story-key resolution** (below). Then **EARLY EXIT** to the appropriate step: `draft` → `the referenced bundled workflow file`, `ready-for-dev`/`in-progress` → `the referenced bundled workflow file`, `in-review` → `the referenced bundled workflow file`. For `done`, ingest as context and proceed to INSTRUCTIONS — do not resume.
...
3. Otherwise — scan artifacts and ask
   - Active specs (`draft`, `ready-for-dev`, `in-progress`, `in-review`) in `the workspace setting `implementation_artifacts``? → List them and HALT. Give the user a choice:
     - Resume one of the listed specs
     - **New** — start new work
     If `draft` selected: Set `spec_file`. Run **Story-key resolution** (below). **EARLY EXIT** → `the referenced bundled workflow file` (resume planning from the draft)
     If `ready-for-dev` or `in-progress` selected: Set `spec_file`. Run **Story-key resolution** (below). **EARLY EXIT** → `the referenced bundled workflow file`
     If `in-review` selected: Set `spec_file`. Run **Story-key resolution** (below). **EARLY EXIT** → `the referenced bundled workflow file`
...
If the spec is an epic story and `the workspace setting `implementation_artifacts`/sprint-status.yaml` exists: find the `development_status` key matching `{epic_num}-{story_num}` by exact numeric equality on the first two segments (so `1-1` never collides with `1-10`). Exactly one match → set `story_key` to that full key. Zero or multiple matches → leave `story_key` unset (warn on multiple).

## INSTRUCTIONS

1. Load context.
   - List files in `the workspace setting `planning_artifacts`` and `the workspace setting `implementation_artifacts``.
   ...
   - **Determine context strategy.** Using the intent and the artifact listing, infer whether the current work is a story from an epic. Do not rely on filename patterns or regex — reason about the intent, the listing, and any epics file content together.
     ...
     **A) Epic story path** — if the intent is clearly an epic story:
     ...
     2. **Check for a valid cached epic context.** Look for `the workspace setting `implementation_artifacts`/epic-<N>-context.md` (where `<N>` is the epic number). A file is **valid** when it exists, is non-empty, starts with `# Epic <N> Context:` (with the correct epic number), and no file in `the workspace setting `planning_artifacts`` is newer.
     ...
     3. **Compile epic context.** Produce `the workspace setting `implementation_artifacts`/epic-<N>-context.md` by following `the referenced bundled workflow file`, in order of preference:
        - **Preferred — subagent:** spawn a subagent synchronously (wait for it to return in this turn) with `the referenced bundled workflow file` as its prompt. Pass it the epic number, the epics file path, the `the workspace setting `planning_artifacts`` directory, and the output path `the workspace setting `implementation_artifacts`/epic-<N>-context.md`.
        - **Fallback — inline** (for runtimes without subagent support, e.g. Copilot, Codex, local Ollama, older Claude): if your runtime cannot spawn subagents, or the spawn fails/times out, read `the referenced bundled workflow file` yourself and follow its instructions to produce the same output file.
     ...
     5. **Previous story continuity.** Regardless of which context source succeeded above, scan `the workspace setting `implementation_artifacts`` for specs from the same epic with `status: done` and a lower story number. Load the most recent one (highest story number below current). Extract its **Code Map**, **Design Notes**, **Spec Change Log**, and **task list** as continuity context for step-02 planning. If no `done` spec is found but an `in-review` spec exists for the same epic with a lower story number, note it to the user and ask whether to load it.
...
2. Clarify intent. Do not fantasize, do not leave open questions. Ask only questions that resolve missing behavior, acceptance criteria, target files, or verification. Safety confirmations are required when existing changes overlap the intended files or make verification ambiguous; optional preferences are not blockers when the implementation path is already clear. If you must ask questions, ask them as a numbered list. When the human replies, verify that every required question was answered. If any were ignored, HALT and re-ask only the missing questions before proceeding. Keep looping until intent is clear enough to implement.
3. Version control sanity check. Does the current branch make sense for this intent — considering its name and recent history? Inspect the working tree before editing. If existing changes overlap the intended files, affect generated outputs needed for verification, or make the branch an obvious mismatch, HALT and ask the human before proceeding. Unrelated tracked or untracked changes are not a blocker: record their paths, preserve them, and continue without modifying them. If version control is unavailable, skip this check.
4. Multi-goal check (see SCOPE STANDARD). If the intent fails the single-goal criteria:
   ...
   - If the user chooses **Split**: For each deferred goal, append one new entry to `the workspace setting `implementation_artifacts`/deferred-work.md` using this format. Do not modify existing entries or look for duplicates. Narrow scope to the first-mentioned goal. Continue routing.
     ```markdown
     - source_spec: none
...
5. Route — choose exactly one:
   ...
   If the explicit spec-folder-plus-story-id pair had no matching story file, keep the colocated `spec_file` selected above. Otherwise, derive a valid kebab-case slug from the clarified intent. If the intent references a tracking identifier (story number, issue number, ticket ID), lead the slug with it (e.g. `3-2-digest-delivery`, `gh-47-fix-auth`). If `the workspace setting `implementation_artifacts`/spec-{slug}.md` already exists: if its status is `draft`, treat it as the same work and resume it (set `spec_file` to that path, **EARLY EXIT** → `the referenced bundled workflow file`); otherwise append `-2`, `-3`, etc. Set `spec_file` = `the workspace setting `implementation_artifacts`/spec-{slug}.md`.

   **a) One-shot** — zero blast radius: no plausible path by which this change causes unintended consequences elsewhere. Clear intent, no architectural decisions.

   **EARLY EXIT** → `the referenced bundled workflow file`

   **b) Plan-code-review** — everything else. When uncertain whether blast radius is truly zero, choose this path.

## NEXT

Read fully and follow `the referenced bundled workflow file`
