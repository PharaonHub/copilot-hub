...
context: [] # optional: `the workspace root/`-prefixed paths to project-wide standards/docs the implementation agent should load. Keep short — only what isn't already distilled into the spec body.
---
...
<!-- How the agent confirms its own work. Prefer CLI commands. For configuration bugs involving YAML or Helm, use a structured parse or rendered-output check that detects duplicate keys or proves the effective value; a text search alone is insufficient. When no CLI check applies, state what to inspect manually. -->
