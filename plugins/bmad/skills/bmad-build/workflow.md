...
the explicit workflow option `activation_steps_prepend`
...
the explicit workflow option `persistent_facts`

### Step 3: Execute Append Steps

Execute each of these steps in order (`_None._` means skip):

the explicit workflow option `activation_steps_append`
...
Read fully and follow: `the referenced bundled workflow file` to begin the workflow.
