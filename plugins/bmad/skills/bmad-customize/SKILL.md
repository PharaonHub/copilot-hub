...
Translate the user's intent into a correctly-placed TOML override file under `the current VS Code workspace root/workspace-local BMAD/custom/` for a customizable agent or workflow skill. Discover, route, author, write, verify.

Scope v1: per-skill `[agent]` overrides (`bmad-agent-<role>.toml` / `.user.toml`) and per-skill `[workflow]` overrides (`bmad-<workflow>.toml` / `.user.toml`). Central config (`the current VS Code workspace root/workspace-local BMAD/custom/config.toml`) is out of scope — point users at the [How to Customize BMad guide](https://docs.bmad-method.org/how-to/customize-bmad/).

When the target's `documented defaults` doesn't expose what the user wants, say so plainly. Don't invent fields.

## Preflight

- No `the current VS Code workspace root/workspace-local BMAD/` → BMad isn't installed. Say so, stop.
Use the explicit inputs supplied in the request and the documented defaults.
- Both present → proceed.

## Activation

Load `workspace-local BMAD/config.toml` and `workspace-local BMAD/config.user.toml` from `the current VS Code workspace root` for `user_name` (default `BMad`) and `communication_language` (default `English`). Greet. If the user's invocation already names a target skill AND a specific change, jump to Step 3.
...
```
uv run this skill directory/a workspace search for SKILL.md files --workspace-root the current VS Code workspace root
```
...
Read the target's `documented defaults`. Top-level `[agent]` or `[workflow]` block defines the surface.
...
Intent outside the exposed surface (step logic, ordering, anything not in `documented defaults`): say so; offer `activation_steps_prepend`/`append` or `persistent_facts` as approximations, or recommend `bmad-builder` to create a custom skill.

## Step 4: Compose the override

Translate plain-English into TOML against the target's `documented defaults` fields. If an existing override was read, frame the change as additive.
...
Overrides are sparse: only the fields being changed. Never copy the whole `documented defaults`.

**Template swap** (`*_template` scalar): offer to copy the default template to `the current VS Code workspace root/workspace-local BMAD/custom/{skill-name}-{purpose}-template.md`, point the override at the new path, offer to help edit it.

## Step 5: Team or user placement

Under `the current VS Code workspace root/workspace-local BMAD/custom/`:
...
3. Write. Create `the current VS Code workspace root/workspace-local BMAD/custom/` if needed.
4. Verify:
   ```
Use the explicit inputs supplied in the request and the documented defaults.
   ```
...
- **Central config** (`the current VS Code workspace root/workspace-local BMAD/custom/config.toml`) — see the [How to Customize BMad guide](https://docs.bmad-method.org/how-to/customize-bmad/).
- **Step logic, ordering, behavior not in `documented defaults`** — open a feature request, or use `bmad-builder` to create a custom skill. Offer to help with either.
- **Skills without a `documented defaults`** — not customizable.
