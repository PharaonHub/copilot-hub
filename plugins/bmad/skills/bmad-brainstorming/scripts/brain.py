...
`--extra PATH` merges a JSON overlay of additional techniques (documented defaults's
...
def load_extra(file: Path) -> list[dict]:
    ...
    documented defaults's `additional_techniques` become first-class across *every*
...
def merge_extra(rows: list[dict], extras: list[dict]) -> list[dict]:
    ...
    documented defaults additional_* entries behave identically across sibling skills."""
    merged = list(rows)
...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument("--extra", type=Path, help="JSON overlay of additional techniques (documented defaults additional_techniques), merged into every command")
