...
Load this when the user signals they're spent or the topic is mined out. `the explicit document workspace/.memlog.md` is the canonical record of the session — everything here derives from it. Communicate in `the user-provided language`; write any document content in `the requested output language`.
...
Record the insights and chosen directions with `uv run "<installed bmad-deep-recon skill directory>/scripts/recon_kit.py" memlog append --workspace <the explicit document workspace> --type insight --text "<insights + chosen directions>"`. **Then run `uv run "<installed bmad-deep-recon skill directory>/scripts/recon_kit.py" memlog set --workspace <the explicit document workspace> --key status --value complete`** — the session is done and must stop being offered for resume. Do this even if the user declines every artifact below.
...
If the session used invented techniques, offer to save a keeper into `the user's explicit additional_techniques input or the documented default` via `bmad-customize` user preferences.

After producing what they chose, offer them ideas for deep-dive brainstorming new sessions, offer to fully extrapolate any ideas into an html report (autonomously brainstorm on their behalf), and most importantly: execute each `the user's explicit external_handoffs input or the documented default` instruction. Then share the artifact paths (and any handoff destinations), invoke `bmad-help` to suggest where this leads next in the BMad ecosystem, let them know if they feel a produced intent is detailed enough they could jump right into passing it to bmad-spec or any other analysis tool (outlined from bmad-help) and run `the user's explicit on_complete input or the documented default` if non-empty.
