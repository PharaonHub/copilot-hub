...
2. Run `the explicit activation_steps_prepend input`. Treat `the explicit persistent_facts input` as foundational context (`file:` entries are loaded).
3. Use the workspace root and user-provided values for name, languages, project name, output folder, and current date.
4. Detect mode. **Headless** when any of: no TTY, programmatic caller (another skill or non-interactive runner), or the first message pre-supplies all inputs and asks for an artifact path back. **Interactive** otherwise. In interactive mode, greet by `the user-provided name` in `the user-provided language`, stay in that language, and mention that `bmad-party-mode` and `bmad-advanced-elicitation` are available for deeper exploration on any field.

Otherwise this is a create, and the first move is the `recon_kit.py memlog init` command documented in Memory and derivation.
 Otherwise this is a create, and the first move is to create or append the run's `.memlog.md`.

Activation is complete. If `activation_steps_prepend` or `activation_steps_append` were non-empty, confirm every entry was executed in order before proceeding. Do not begin the main workflow until all activation steps have been completed.

## Workspace

The spec is **always a folder** named `the explicit spec_output_path input/the explicit run_folder_pattern input`, resolving by default to `{output_folder}/specs/spec-{slug}/`.
...
Writes go through the shared script — `uv run "<installed bmad-deep-recon skill directory>/scripts/recon_kit.py" memlog` (atomic; never read it back except to resume):

- `uv run "<installed bmad-deep-recon skill directory>/scripts/recon_kit.py" memlog init --workspace {spec-folder} --field topic="<what is being specced>"` — once, at create.
- `uv run "<installed bmad-deep-recon skill directory>/scripts/recon_kit.py" memlog append --workspace {spec-folder} --type <decision|constraint|capability|assumption|question|direction|note|event> --text "<one-line gist, reason included>"` — as each lands.
- Terminal moments (a validation verdict, "spec finalized") are `--type event` entries; the memlog carries no status field.

## The Operation

Read the input and its ancillary linked materials. If there is no input, follow the no-input branch in **Workspace** (ask or block). If a prior `.memlog.md` exists at the target folder, read it — the operation becomes an update, and the memlog (not the rendered `SPEC.md`) is the authority on what was decided and on capability IDs. Preserve those IDs; new capabilities get the next unused `CAP-N`; never reuse retired IDs. Otherwise this is a create, and the first move is the concrete `recon_kit.py memlog init` command above.
...
Run `the explicit on_complete input` if set.
