...
Append memory entries directly to the workspace `.memlog.md` file (don't read it back except on resume):

- `uv run "<installed bmad-deep-recon skill directory>/scripts/recon_kit.py" memlog init --workspace <the explicit document workspace> --field scope="..." --field purpose="..." --field altitude="..."`
- `uv run "<installed bmad-deep-recon skill directory>/scripts/recon_kit.py" memlog append --workspace <the explicit document workspace> --type <decision|constraint|version|assumption|question|direction|event> --text "..."`

## Resolution rules

- Bare paths (e.g. `references/headless.md`) resolve from this skill's bundled directory.
- `the workspace root` → the project working directory; `{skill-name}` → the skill directory's basename.
- Optional workflow behavior comes from explicit caller inputs; `the explicit document workspace` is the bound run folder.
- Forward slashes only. Config variables already contain `the workspace root` in their resolved values — never double-prefix.

## On Activation

**Forwarded activation:** if a caller invoked you with a stated intent and explicit inputs, honor them verbatim and resolve only the remaining inputs from the conversation or workspace.

1. Read optional behavior from explicit caller inputs and use bundled assets as defaults. Run any supplied activation steps, hold any supplied persistent facts as standing context, and consult any supplied external sources on demand.
2. Use the workspace root and user-provided values for name, languages, project name, planning-artifacts directory, and current date; missing values take neutral defaults, never block.
3. Headless (no interactive user) → follow `references/headless.md` for the whole run. Otherwise greet `the user-provided name` in `the user-provided language`. Detect the intent from the conversation and input — **create** (the default), **update** an existing spine, or **validate** one (see those sections). If the real ask is requirements / UX / a capability contract / epic breakdown / an agent, invoke the `bmad-prd`, `bmad-ux`, `bmad-spec`, `bmad-create-epics-and-stories`, or `bmad-workflow-builder` (if the BMad Builder module is installed) skill instead.
4. If a run folder for this target already exists under `the explicit spine_output_path input`, offer to resume from its memlog rather than restart.
5. Interactive create: offer the working mode in `the user-provided language` — **Coaching path** (default) or **Fast path** (see *How you work*) — before any drafting; default to Coaching unless the user asks for speed.
...
For a new spine, bind `the explicit document workspace` to the explicit spine output location, seed `ARCHITECTURE-SPINE.md` from the explicit spine template input, initialize session memory, and tell the user the path. **At epic altitude, scope the folder to the epic** so per-epic runs don't collide.

## Reviewer Gate

The spine's pre-handoff review — full mechanics in `references/reviewer-gate.md`. Load it when finalizing or validating: a deterministic `lint_spine.py` pass, then a rubric walker (good-spine checklist) + every `the explicit finalize_reviewers input` lens dispatched as parallel subagents against `ARCHITECTURE-SPINE.md`, scaled to stakes. At Finalize you apply the clear fixes; under the Validate intent you deliver a bespoke HTML report and then get user input.
...
5. **Renderings & polish.** The spine is the build deliverable; with it and the memlog now in place, produce any *additional* human-facing artifact the user needs, scoped to the purpose and audience drawn out up front. The up-front question already flagged whether one's needed; if it wasn't, still offer one here, seeding concrete options: an interactive HTML+SVG deck to walk a team through the architecture and drive discussion, a fuller HTML/md solution design, a C4 set, or a view of how the work splits across teams/epics. Build only what they pick, right-sized to that purpose; apply `the explicit doc_standards input` polish to that prose only, never to the spine.
6. **External handoffs.** Run `the explicit external_handoffs input`; surface returned URLs/IDs. Offer to invoke the `bmad-spec` skill to adopt the spine as a companion, keeping `AD` IDs stable so downstream can cite them.
7. **Close.** Set the spine's own frontmatter `status: final`, `updated: the current date`; record a finalization event in session memory (it has no status field). Share paths. Next, **lead with `bmad-spec`** — recommend adopting/refreshing the spine as a spec companion (always the top recommendation when a spec was an input, and a useful next step even when it wasn't), then `bmad-create-epics-and-stories` or — epic altitude — `bmad-build`; or invoke `bmad-help` to route.
8. Run `the explicit on_complete input`.
